...
export default {
  ...
  setup() {
    ...
    const updateChartData = (stats) => {
      ...
      cpuRef.value.chart.data.labels = [...(cpuRef.value?.chart?.data?.labels.slice(-linePoints.value) || [0]), dateLabel]
      cpuRef.value.chart.data.datasets[0].data = [...(cpuRef.value?.chart?.data?.datasets[0].data.slice(-linePoints.value) || [0]), cpuUsage.toFixed(2)]
      cpuRef.value?.chart?.update()

      memoryRef.value.chart.data.labels = [...(memoryRef.value?.chart?.data?.labels.slice(-linePoints.value) || [0]), dateLabel]
      memoryRef.value.chart.data.datasets[0].data = [...(memoryRef.value?.chart?.data?.datasets[0].data.slice(-linePoints.value) || [0]), stats.memory_stats.usage / 1024 / 1024]
      memoryRef.value.chart.data.datasets[1].data = [...(memoryRef.value?.chart?.data?.datasets[1].data.slice(-linePoints.value) || [0]), stats.memory_stats.limit / 1024 / 1024]
      memoryRef.value?.chart?.update()

      if (blkioStats) {
        blkioRef.value.chart.data.labels = [...(blkioRef.value?.chart?.data?.labels.slice(-linePoints.value) || [0]), dateLabel]
        blkioRef.value.chart.data.datasets[0].data = [...(blkioRef.value?.chart?.data?.datasets[0].data.slice(-linePoints.value) || [0]), blkioStats[0].value]
        blkioRef.value.chart.data.datasets[1].data = [...(blkioRef.value?.chart?.data?.datasets[1].data.slice(-linePoints.value) || [0]), blkioStats[1].value]
        blkioRef.value?.chart?.update()
      }

      networkRef.value.chart.data.labels = [...(networkRef.value?.chart?.data?.labels.slice(-linePoints.value) || [0]), dateLabel]
      networkRef.value.chart.data.datasets[0].data = [...(networkRef.value?.chart?.data?.datasets[0].data.slice(-linePoints.value) || [0]), networkRx]
      networkRef.value.chart.data.datasets[1].data = [...(networkRef.value?.chart?.data?.datasets[1].data.slice(-linePoints.value) || [0]), networkTx]
